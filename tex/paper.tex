...
\end{equation}
Since the slot jet is directed \emph{downwards}, the sealing is strongest near the floor
($z\simeq 0$) and decays toward the top ($z=h$). We therefore adopt a
monotone \emph{decreasing} profile
\begin{equation}
  \Phi(\zeta)= (1-\zeta)^{m}, \qquad \zeta=z/h\in[0,1],\quad m\gtrsim 1,
\end{equation}
so that $\Phi(0)=1$ and $\Phi(1)=0$.
...
\end{equation}
with BCs: symmetry at $r=0$; no-normal-flow at $z=0$ and $z=h$; and a
\emph{rim pressure} at $r=R^{-}$ induced by the turned curtain,
\begin{equation}
  p_{\mathrm{edge}}(z)=p_0 + \Delta p\,\Phi\!\left(\frac{z}{h}\right),\qquad
  \Delta p = C_t\,\frac{\rho_j U_{\mathrm{out}}^2\,b}{h_{\mathrm{eff}}},\quad
  \Phi(\zeta)=(1-\zeta)^m.
\end{equation}
Note that the downward-oriented curtain motivates a decreasing profile $\Phi$; we do
\emph{not} rely on the sign of $\partial_z p$ to prescribe a particular $w$ at the
boundaries, which remain impermeable in the reduced model.
...
\begin{enumerate}
  ...
  \item Compute the area-mean cushion pressure
  \(
    \overline{p}=\frac{1}{\pi R_{\mathrm{tot}}^2}\!
    \int_0^{R^{-}}\!\!\int_0^h (p-p_0)\,2\pi r\,dz\,dr.
  \)
  \item If $|\overline{p}-p_c|>\varepsilon$, update the curtain intensity and repeat.
