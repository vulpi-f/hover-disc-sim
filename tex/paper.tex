...
\end{equation}
Because the slot jet is issued \emph{downwards}, the jet is most energetic near the
slot (top, $z=h$) and less effective near the floor ($z=0$). In this reduced model
$\Phi$ represents the \emph{sealing effectiveness} (not the static pressure inside the
jet), therefore it should be \emph{increasing} with height:
\begin{equation}
  \Phi(\zeta)= \zeta^{m}, \qquad \zeta=z/h\in[0,1],\ \ m\gtrsim 1,
\end{equation}
so that $\Phi(0)=0$ (weak sealing near the floor) and $\Phi(1)=1$ (strong sealing
near the slot). On $z=0$ and $z=h$ we impose no-normal-flow in the reduced core model
\begin{equation}
  \partial_z p(r,0)=\partial_z p(r,h)=0,
\end{equation}
while $r=0$ enforces symmetry $\partial_r p(0,z)=0$. The inner make-up jet is
\emph{not} imposed as a local boundary condition; its effect enters through the global
mass balance (Sec.~\ref{sec:shooting}).
\paragraph{Remark on $\Phi$: sealing vs static pressure.}
The increasing profile $\Phi(\zeta)=\zeta^m$ is chosen to represent the \emph{sealing
effectiveness} of the downward curtain (barrier highest near the slot). The local static
pressure inside the jet would show the opposite trend (lower near the slot, higher near
the floor), but the reduced core model uses a rim \emph{sealing} pressure as boundary
data rather than the jet's internal static distribution.
